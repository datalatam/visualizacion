...
datos_judiciales <- read_excel("datos/PJCROD_POLICIALES_V1-2019.xls") %>% 
  ...
  mutate_at(vars(canton, distrito), ~str_replace(., " De ", " de "))
```
...
datos_poblacion <- datos_poblacion %>% 
  ...
  rename_at(vars(-distrito), ~paste0("a", .))
```
...
for (i in 1:nrow(datos_poblacion)) {
  ...
  if (suma_distritos == 0) {
    suma_distritos <- datos_poblacion$a2011[i]
    datos_poblacion$es_canton[i] <- TRUE
  } else {
    suma_distritos <- suma_distritos - datos_poblacion$a2011[i]  
  }
