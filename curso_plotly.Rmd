...
resumen <- datos_cr %>% 
  ...
  tally(name = "delitos_x_genero") %>%
  group_by(mes, provincia, delito) %>% 
  mutate(delitos_por_provincia = sum(delitos_x_genero),
         prop_genero = delitos_x_genero / delitos_por_provincia) %>% 
  filter(genero == "Mujer")
...
gg <- ggplot(resumen, aes(x = prop_genero, y = delitos_por_provincia,
  ...
  xlab("Proporción del género") + ylab("Delitos por provincia") +
