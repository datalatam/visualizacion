...
style(p, hoverinfo = "none", traces = 1:2)
```
...
gg <- ggplot(prop_provincia, aes(x = cantidad, y = pob2016, color = provincia)) +
  geom_point(aes(size = prop_delitos, frame = mes), alpha = 0.7) +
  scale_color_viridis_d() +
  xlab("Cantidad de delitos") + ylab("Población en año 2016") +
...
```{r delay}
base <- prop_provincia %>% 
  plot_ly(x = ~cantidad, y = ~pob2016, size = ~prop_delitos,
          text = ~provincia, hoverinfo = "text") %>% 
  layout(xaxis = list(title = "Cantidad de delitos"),
         yaxis = list(title = "Población en año 2016"))
