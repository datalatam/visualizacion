...
datos_cr %>% 
  ...
  #scale_y_continuous(labels = scales::comma, breaks = seq(from = 0, to = 2000, by = 200))
  #scale_x_discrete(labels = c("Jul", "Ago", "Set", "Oct", "Nov", "Dic"))
```

Para cerrar, hasta le podemos cambiar el tipo y tamaño de letra.
...
datos_cr %>% 
  ...
  group_by(delito, provincia) %>% 
  tally() %>% 
  ggplot(aes(x = stringr::str_wrap(delito, 10),
             y = provincia)) +
  geom_point(aes(size = n)) +
  geom_text(aes(label = n,
                hjust = -.4, vjust = -.4),
